...
def upgrade() -> None:
    # Idempotent by table: a database that already has some (but not all) of
    # these tables — e.g. a partial prior run, or a volume that predates this
    # migration being stamped — must not crash-loop on "relation already
    # exists". Each block below is skipped individually when its table is
    # already present, so `docker compose up` always converges in one shot.
    ...
    # ------------------------------------------------------------------
    if "marketing_agent_runs" not in existing_tables:
        op.create_table(
            "marketing_agent_runs",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
            sa.Column("organization_id", postgresql.UUID(as_uuid=True), nullable=False),
    ...
    # sales_agent_runs
    # ------------------------------------------------------------------
    if "sales_agent_runs" not in existing_tables:
        op.create_table(
            "sales_agent_runs",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
            sa.Column("organization_id", postgresql.UUID(as_uuid=True), nullable=False),
    ...
    # support_agent_runs
    # ------------------------------------------------------------------
    if "support_agent_runs" not in existing_tables:
        op.create_table(
            "support_agent_runs",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
            sa.Column("organization_id", postgresql.UUID(as_uuid=True), nullable=False),
    ...
    # agent_webhooks
    # ------------------------------------------------------------------
    if "agent_webhooks" not in existing_tables:
        op.create_table(
            "agent_webhooks",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
            sa.Column("organization_id", postgresql.UUID(as_uuid=True), nullable=False),
    ...
    # handoff_requests
    # ------------------------------------------------------------------
    if "handoff_requests" not in existing_tables:
        op.create_table(
            "handoff_requests",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
            sa.Column("organization_id", postgresql.UUID(as_uuid=True), nullable=False),
    ...
    # provider_usage_logs
    # ------------------------------------------------------------------
    if "provider_usage_logs" not in existing_tables:
        op.create_table(
            "provider_usage_logs",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
