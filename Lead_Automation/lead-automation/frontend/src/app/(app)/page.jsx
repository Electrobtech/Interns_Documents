...
function StatCard({ label, value, delta, icon: Icon, href, tone = 'blue' }) {
  const TONES = {
    blue:    { bg: 'from-blue-50 to-indigo-100',   icon: 'text-blue-600',   val: 'text-blue-700'   },
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      <Link href="/onboarding"
        className="flex items-center justify-between bg-blue-50 border border-blue-100 rounded-2xl px-5 py-3.5 hover:bg-blue-100/60 transition-colors group">
        <div className="flex items-center gap-3">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Conversations" value={(data?.totalConversations ?? 0).toLocaleString('en-IN')} delta="12.5%" icon={MessageCircle} href="/inbox" tone="blue" />
        <StatCard label="Revenue Impact"      value={inr(data?.revenueImpact)}    delta="22.4%"  icon={ShoppingCart} href="/ecommerce" tone="emerald" />
        <StatCard label="Open Conversations"  value={data?.openConversations ?? 0} icon={Inbox}   href="/inbox"     tone="amber" />
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          ...
          <Link href="/ai-agents"
            className="ml-auto text-xs font-semibold text-blue-600 hover:text-blue-700 flex items-center gap-1">
        ...
        <div className="divide-y divide-slate-50">
          ...
          {inbox.map((c, i) => (
            <div key={i} className="flex items-center gap-3 py-3">
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-100 to-indigo-200 grid place-items-center text-xs font-bold text-blue-700 shrink-0">
