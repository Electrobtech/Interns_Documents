...
A comprehensive lead automation platform with multi-channel communication, marketing hub, AI agents, and CRM capabilities.
...
**Important:** Seed data is automatically loaded on first startup. If you restart Docker, the seed data persists.
...
If you don't see the expected seed data after running the setup:
...
2. **Restart the seed-data-init container:**
   ```bash
   docker-compose restart seed-data-init
   ```

3. **Manually re-seed the database:**
...
4. **Complete database reset (last resort):**
   ```bash
   docker-compose down
...
The seed data is crucial for your friends to see a working system with:
- Pre-configured users and teams
- Sample contacts and leads
- Marketing campaigns and audiences
- Communication channels
- Integration examples

Without seed data, the application would appear empty and require manual configuration to demonstrate its capabilities.

### How Automatic Seed Data Works

The `seed-data-init` container automatically checks for seed data when PostgreSQL is ready. If the database is empty, it loads the seed data automatically. This ensures that:
- First-time users always see populated data
- Container restarts don't lose the seed data
- Development environments stay consistent
