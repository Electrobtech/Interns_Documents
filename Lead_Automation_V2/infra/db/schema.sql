...
-- ---------- Marketing Hub Service Tables (mh_*) ----------
CREATE TABLE IF NOT EXISTS mh_audiences (
  ...
  size_cached     INT NOT NULL DEFAULT 0,
  member_count    INT NOT NULL DEFAULT 0,
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_mh_audiences_org ON mh_audiences (organization_id);
...
CREATE TABLE IF NOT EXISTS mh_delivery_events (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  campaign_id     UUID NOT NULL REFERENCES mh_campaigns(id) ON DELETE CASCADE,
  recipient_id    UUID REFERENCES mh_recipients(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_mh_delivery_events_campaign ON mh_delivery_events (campaign_id);
