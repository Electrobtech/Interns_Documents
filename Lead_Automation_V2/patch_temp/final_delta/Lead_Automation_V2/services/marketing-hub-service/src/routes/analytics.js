...
router.get('/dashboard', authenticate, requirePermission('campaigns:read'), async (req, res) => {
  try {
    ...
    `, [req.user.organizationId, dateRange.start]);
...
router.get('/campaigns/:id', authenticate, requirePermission('campaigns:read'), async (req, res) => {
  try {
...
router.get('/audiences', authenticate, requirePermission('campaigns:read'), async (req, res) => {
  try {
...
router.get('/channels/comparison', authenticate, requirePermission('campaigns:read'), async (req, res) => {
  try {
    const { period = '30d' } = req.query;
    const dateRange = getDateRange(period);
...
router.get('/export', authenticate, requirePermission('campaigns:read'), async (req, res) => {
  try {
