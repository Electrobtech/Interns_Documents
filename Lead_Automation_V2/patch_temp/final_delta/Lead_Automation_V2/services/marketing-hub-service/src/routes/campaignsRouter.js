...
function buildRouter(kind) {
  ...
  // "AI Optimize" toolbar button — real LLM call over the campaign's own
  // current metrics (no fabricated numbers), returns a structured
  // suggestion the drawer can render. Doesn't write anything back on its
  // own; a human still decides whether to apply it.
  // "AI Optimize" — tries a real LLM call over the campaign's own metrics
  // first (this is the one place you said you DO have an API key for),
  // and only drops to the deterministic heuristic below if the LLM call
  // itself fails (bad/missing key, provider outage, rate limit). Response
  // shape is always { suggestions: [{ field, action }], summary?, source }
  // so the frontend (OptimizePanel / toolbar) never has to branch on which
  // path answered.
  router.post('/:id/optimize', canWrite, ah(async (req, res) => {
    ...
    const metrics = { total, failRate, readRate, replyRate };
    ...
    const system = [
      'You are a marketing-campaign performance analyst.',
      'Given a campaign\'s real metrics, respond with ONLY a JSON object of the exact shape:',
      '{"summary": string, "suggestions": [{"area": "budget"|"audience"|"copy"|"timing", "suggestion": string}]}',
      'Give 2-4 suggestions, each one concrete sentence. Base every claim strictly on the metrics given — do not invent numbers, and say so plainly if there is not yet enough data (e.g. zero recipients).',
    ].join(' ');
    const userPrompt = JSON.stringify({
      name: c.name, objective: c.objective, channel: c.channel, status: c.status,
      budget_amount: c.budget_amount, total_recipients: total,
      sent: c.sent_count || 0, delivered: c.delivered_count || 0, read: c.read_count || 0,
      replied: c.replied_count || 0, failed: c.failed_count || 0, failRate, readRate, replyRate,
    });

    let payload;
    try {
      const llm = await completeJSON(system, userPrompt, { temperature: 0.4, maxTokens: 500 });
      const suggestions = (llm.suggestions || []).map((s) => ({ field: s.area || null, action: s.suggestion }));
      payload = { suggestions: suggestions.length ? suggestions : heuristicSuggestions(), summary: llm.summary, source: 'llm', metrics };
    } catch (err) {
      console.error('[campaigns.optimize] LLM call failed, using heuristic fallback:', err.message);
      payload = { suggestions: heuristicSuggestions(), source: 'heuristic', metrics };
    }

    logAudit(req, `mh_${kind}.optimize`, { id: c.id, source: payload.source });
    res.json({ campaignId: c.id, generatedAt: new Date().toISOString(), ...payload });
  }));
