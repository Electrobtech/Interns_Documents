...
router.get('/:category', authenticate, requirePermission('campaigns:read'), async (req, res) => {
  try {
...
// Update setting
router.put('/:category/:key', authenticate, requirePermission('campaigns:write'), async (req, res) => {
  try {
    ...
    const result = await query(
      `INSERT INTO mh_settings (organization_id, category, key, value, description, is_public)
       ...
       RETURNING *`,
      [req.user.organizationId, category, key, JSON.stringify(value), description, is_public || false]
    );
...
// Delete setting
router.delete('/:category/:key', authenticate, requirePermission('campaigns:write'), async (req, res) => {
  try {
    const { category, key } = req.params;
    
    const result = await query(
      'DELETE FROM mh_settings WHERE organization_id = $1 AND category = $2 AND key = $3',
      [req.user.organizationId, category, key]
    );
...
router.get('/integrations/list', authenticate, requirePermission('campaigns:read'), async (req, res) => {
  try {
...
router.post('/integrations', authenticate, requirePermission('campaigns:write'), async (req, res) => {
  try {
    ...
    const result = await query(
      ...
      [
        req.user.organizationId, provider, service_type, JSON.stringify(credentials),
...
// Update integration
router.put('/integrations/:id', authenticate, requirePermission('campaigns:write'), async (req, res) => {
  try {
...
// Delete integration
router.delete('/integrations/:id', authenticate, requirePermission('campaigns:write'), async (req, res) => {
  try {
    const result = await query(
...
router.post('/integrations/:id/test', authenticate, requirePermission('campaigns:write'), async (req, res) => {
  try {
...
router.get('/defaults/:category', authenticate, requirePermission('campaigns:read'), async (req, res) => {
  try {
    const { category } = req.params;
...
router.post('/integrations/:id/sync', authenticate, requirePermission('campaigns:write'), async (req, res) => {
  try {
    const integration = await query(
      'SELECT * FROM mh_integrations WHERE id = $1 AND organization_id = $2',
      [req.params.id, req.user.organizationId]
