...
// Get calendar events
router.get('/events', authenticate, requirePermission('campaigns:read'), async (req, res) => {
  try {
...
router.post('/events', authenticate, requirePermission('campaigns:write'), async (req, res) => {
  try {
    const { 
    ...
    const result = await query(
      ...
      [
        req.user.organizationId, title, description, event_type, start_date, end_date,
...
// Get event by ID
router.get('/events/:id', authenticate, requirePermission('campaigns:read'), async (req, res) => {
  try {
    const result = await query(
...
router.put('/events/:id', authenticate, requirePermission('campaigns:write'), async (req, res) => {
  try {
    const { 
      title, description, event_type, start_date, end_date, all_day,
...
router.delete('/events/:id', authenticate, requirePermission('campaigns:write'), async (req, res) => {
  try {
    const result = await query(
...
router.get('/view/:view_type', authenticate, requirePermission('campaigns:read'), async (req, res) => {
  try {
    ...
    const result = await query(
      ...
      [req.user.organizationId, startDate.toISOString(), endDate.toISOString()]
    );
...
router.get('/deadlines/upcoming', authenticate, requirePermission('campaigns:read'), async (req, res) => {
  try {
    ...
    const result = await query(
      `SELECT * FROM mh_calendar_events 
       ...
       ORDER BY start_date ASC`,
      [req.user.organizationId, endDate.toISOString()]
    );
  ...
  try {
    ...
    const stats = await query(
      `SELECT 
       ...
       ORDER BY count DESC`,
      [req.user.organizationId, startDate.toISOString()]
    );
