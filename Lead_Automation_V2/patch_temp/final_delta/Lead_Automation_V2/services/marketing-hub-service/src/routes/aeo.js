...
router.post('/', authenticate, requirePermission('campaigns:write'), async (req, res) => {
  try {
    ...
    const result = await query(
      ...
      [
        req.user.organizationId, searchQuery, answer_type, current_content,
  ...
  try {
    ...
    const result = await query(
      `UPDATE mh_aeo_optimization 
       ...
       RETURNING *`,
      [optimized_content, status, JSON.stringify(performance || {}), req.params.id, req.user.organizationId]
    );
...
router.post('/analyze', authenticate, requirePermission('campaigns:read'), async (req, res) => {
  try {
...
// Get AEO insights
router.get('/insights/overview', authenticate, requirePermission('campaigns:read'), async (req, res) => {
  try {
...
// Real LLM call for the actual optimized answer text (the one thing worth
// an LLM here); tips + estimated_visibility stay deterministic/heuristic
// since "estimated visibility" isn't something even a real AI-search
// crawler could honestly report without live citation data.
async function generateAEOOptimization(searchQuery, answerType, currentContent) {
  const answerTypeGuides = {
    featured_snippet: 'Write a direct, self-contained answer under 58 words that could be lifted verbatim into a Google Featured Snippet. Lead with the exact question restated as a bolded heading, then the answer, then 3 short bullet points of supporting detail.',
    local_pack: 'Write content structured around Name/Address/Phone/Hours placeholders in [brackets] plus 1-2 sentences of locally-relevant business description.',
    knowledge_panel: 'Write a concise, encyclopedic-style factual summary (2-3 sentences) suitable for an AI knowledge panel, plus 2 related facts.',
  };
  const guide = answerTypeGuides[answerType] || 'Write a direct, factual, well-structured answer optimized to be cited by AI answer engines (ChatGPT, Perplexity, Gemini, Google AI Overviews).';

  const system = [
    'You optimize web content so AI answer engines are more likely to cite it directly.',
    guide,
    'Output ONLY the optimized answer content itself (plain text/markdown) — no preamble, no explanation.',
  ].join(' ');
  const userPrompt = currentContent
    ? `Query: "${searchQuery}"\n\nExisting content to improve:\n${currentContent}`
    : `Query: "${searchQuery}"\n\nNo existing content — write this from scratch.`;
  ...
  try {
    const optimized_content = (await complete(system, userPrompt, { temperature: 0.5, maxTokens: 400 })).trim();
    return {
      optimized_content,
      tips: tipsByType[answerType] || tipsByType.default,
      performance: { estimated_visibility: null, generated_by: 'llm' },
    };
  } catch (err) {
    console.error('[aeo.generate] LLM call failed, using template fallback:', err.message);
    // Visibly-labeled fallback, not a silent swap — matches content.js's pattern.
    const fallback = {
      featured_snippet: `**${searchQuery}**\n\n${currentContent || 'Optimized answer content goes here.'}\n\nKey points:\n• Bullet point 1\n• Bullet point 2\n• Bullet point 3`,
      local_pack: `${currentContent || 'Local business information'}\n\nLocation: [Your Business Address]\nHours: [Business Hours]\nPhone: [Phone Number]`,
    }[answerType] || currentContent || 'Optimized content for answer engines';
    return {
      optimized_content: fallback,
      tips: tipsByType[answerType] || tipsByType.default,
      performance: { estimated_visibility: null, generated_by: 'template_fallback' },
    };
  }
}
