...
BEGIN
  FOREACH t IN ARRAY ARRAY[
    ...
    -- Added: these 11 tables (services/marketing-hub-service's newer
    -- routes — content/analytics/assets/templates/seo/aeo/competitors/
    -- calendar/knowledge/settings/integrations) shipped with schema but
    -- were never added here, so they ran with RLS off entirely (no
    -- isolation at all, not even fail-closed) until this fix.
    'mh_assets', 'mh_templates', 'mh_content_studio',
    'mh_seo_keywords', 'mh_seo_audits', 'mh_aeo_optimization',
    'mh_competitors', 'mh_calendar_events', 'mh_knowledge_articles',
    'mh_settings', 'mh_integrations'
  ]
  LOOP
