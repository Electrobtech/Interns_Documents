...
import { Search, SlidersHorizontal, Clock, Plug } from 'lucide-react';
import { useApi } from '@/lib/useApi';
import { useSocketEvent } from '@/lib/socket';
import ChatListItem from '@/components/whatsapp/ChatListItem';
...
// Same QuickReply.ai-style master-detail layout as
// components/whatsapp/WhatsAppConversationsView.jsx, generalized to take
// `type` (channel_type key, e.g. 'instagram') and `meta` ({ label, icon })
// as props so it can be reused for Instagram, Messenger, Web Chat, and
// Voice Call without duplicating the whole view per channel. WhatsApp keeps
// its own dedicated component untouched; LinkedIn, Email, and SMS/RCS keep
// their own bespoke pages/panels and never render this component.
export default function ChannelConversationsView({ type, meta }) {
  const { call } = useApi();
  ...
  useEffect(() => { load(); }, [load]);
  // Reset the selected thread when switching channels (e.g. sidebar nav
  // from Instagram straight to Messenger) so stale content can't flash.
  useEffect(() => { setSelectedId(null); }, [type]);
    ...
    <div className="flex flex-col" style={{ height: 'calc(100vh - 4rem)' }}>
      ...
      <div className="flex-1 flex min-h-0">
        ...
        {selectedId ? (
          ...
          <EmptyChatState channelLabel={meta.label} />
        )}
      </div>
    </div>
  );
