'use client';
/**
 * MagicCard — a reusable extraction of the React Bits "MagicBento" effect
 * (spotlight, border-glow, tilt, magnetism, particles, click-ripple),
 * refactored so it can wrap ANY existing card content instead of only
 * the component's own demo grid.
 *
 * Usage:
 ...
 *     <GlobalSpotlight gridRef={gridRef} enabled={!isMobile} glowColor="37, 99, 235" />
 *     <ParticleCard className="..." glowColor="37, 99, 235" disableAnimations={isMobile}>
...
export const DEFAULT_PARTICLE_COUNT = 8;
export const DEFAULT_SPOTLIGHT_RADIUS = 300;
export const DEFAULT_GLOW_COLOR = '37, 99, 235'; // brand blue, rgb triplet
...
/**
 * Wraps arbitrary children with the particle / tilt / magnetism / click-ripple
 * effects. `disableAnimations` should be true on mobile — callers pass the
 * result of useMobileDetection() (or their own `disableAnimations || isMobile`
 * condition) so every card consistently satisfies that requirement.
 */
...
/**
 * Renders the single fixed spotlight that follows the cursor across a whole
 * `.bento-section` and updates each `.magic-bento-card`'s glow intensity by
 * proximity. Mount once per section (not once per card).
 */
