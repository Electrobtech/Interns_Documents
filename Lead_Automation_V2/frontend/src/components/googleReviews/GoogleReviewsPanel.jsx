...
export default function GoogleReviewsPanel() {
  ...
  const handleSaveConfig = async (clientId, clientSecret) => {
    const ok = await g.saveConfig(clientId, clientSecret);
    if (ok) toast.success('Google OAuth credentials saved.');
    else toast.error(g.error || 'Failed to save Google OAuth credentials.');
    return ok;
  };

  const handleConnect = async () => {
    const ok = await g.connect();
    if (!ok) toast.error(g.error || 'Failed to start Google connection.');
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <GoogleConfigPanel
        config={g.config}
        configLoading={g.configLoading}
        savingConfig={g.savingConfig}
      ...
      <ConnectionPanel
        ...
        busy={g.busy}
        configured={!!g.config?.configured}
