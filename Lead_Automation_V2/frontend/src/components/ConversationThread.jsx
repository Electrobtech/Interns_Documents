...
// Renders a single conversation's transcript + reply box. Used both at
// /app/inbox/[id] (the unified inbox) and at
// /app/channels/[type]/conversation/[id] (per-channel view, e.g.
// WhatsApp > Conversations > a conversation) so opening a WhatsApp chat
// stays under Channels > WhatsApp instead of jumping into the unified
// inbox. `backHref`/`backLabel` control where the back link goes; the
// conversation id itself comes from the route (`useParams().id`).
export default function ConversationThread({ backHref = '/app/inbox', backLabel = 'Back to inbox' }) {
  const { id } = useParams();
  const { call } = useApi();
  ...
  // Live updates — see services/inbox-service/src/realtime.js. Covers a
  // real inbound WhatsApp message arriving mid-view just as much as our
  // own sends below (which also call load() themselves), so a customer
  // typing something while this thread is open shows up without a refresh.
  ...
  return (
    <div className="p-6 max-w-3xl">
      <Link href={backHref} className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-brand mb-4">
