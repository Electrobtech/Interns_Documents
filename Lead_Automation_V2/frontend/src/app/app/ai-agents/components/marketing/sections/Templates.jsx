...
import { Layers, Plus, Trash2, Copy, Check, Loader2 } from 'lucide-react';

import {
  useTemplates, useCreateTemplate, useDeleteTemplate, CONTENT_TYPES, BROADCAST_CHANNELS,
            ...
            <Card key={t.id} hover className="p-4 flex flex-col">
              ...
              <div className="flex items-center justify-between gap-2 mt-3 pt-3 border-t border-[#EEF1F6]">
                <span className="text-[10px] text-slate-400">
                  {t.channel ? `${t.channel} · ` : ''}used {t.usage_count}× · {timeAgo(t.created_at)}
                </span>
    ...
    >
      ...
      {variables.length > 0 && (
        <div className="flex flex-wrap gap-1.5 -mt-2 mb-4">
          {variables.map((v) => (
            <span key={v} className="font-mono text-[10px] px-1.5 py-0.5 rounded bg-violet-50 text-violet-600">
              {`{{${v}}}`}
            </span>
          ))}
        </div>
