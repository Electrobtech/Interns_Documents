...
// Maps a GST "Constitution of Business" string (as returned by the
// verify-gst endpoint) to our internal business_type enum, for GST-verified
// auto-fill of Step 2. Returns null for anything unrecognized so callers can
// leave the user's existing selection untouched rather than clearing it.
export function mapGstConstitution(raw) {
  const s = (raw || '').toLowerCase();
  if (!s) return null;
  if (s.includes('proprietor')) return 'proprietorship';
  if (s.includes('llp') || s.includes('limited liability partnership')) return 'llp';
  if (s.includes('partnership')) return 'partnership';
  if (s.includes('private limited') || s.includes('pvt')) return 'private_limited';
  if (s.includes('public limited')) return 'public_limited';
  if (s.includes('trust') || s.includes('society') || s.includes('ngo')) return 'ngo';
  return null;
}
...
// Step 1 — GST Verification. Format-checked only if a number was entered;
// verification itself (the "Verify GST" button) is not force-required so
// people can back out to "I don't have a GST Number" at any time.
export function validateGst(gst) {
  const e = {};
  if (gst.hasGst === 'yes' && gst.gstNumber && !isGst(gst.gstNumber)) e.gstNumber = 'Invalid GST format';
  return e;
}
