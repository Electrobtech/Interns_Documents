...
class ContentDoc(OrbqTable):
    ...
    # Indices into `claims_requiring_verification` a human has signed off on.
    # Stored as indices rather than copies of the claim text so a reworded
    # claim doesn't silently keep its old verification.
    claims_verified: Mapped[list] = mapped_column(
        JSONB, nullable=False, default=list, server_default=text("'[]'::jsonb")
    )
...
class ContentVersion(OrbqTable):
    """Immutable snapshot of a ContentDoc at one version.

    `ContentDoc.version` was already being incremented on every save, but with
    nowhere to look up what a prior version said — a counter implying a history
    that did not exist. A row is written here inside the same transaction as
    the increment, so the two can never disagree.
    """
    ...
    content_id: Mapped[uuid.UUID] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("marketing_content.id", ondelete="CASCADE"),
        nullable=False,
    )
    ...
    __table_args__ = (
        Index("uq_content_version", "content_id", "version", unique=True),
        Index("ix_content_versions_content", "content_id", "version"),
    )
