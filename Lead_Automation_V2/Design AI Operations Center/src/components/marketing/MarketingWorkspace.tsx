...
const TABS = [
  'Overview', 'Campaign Studio', 'Content Studio', 'SEO', 'AEO',
  'Personas', 'Competitor Intel', 'CTWA Ads', 'Calendar', 'Analytics',
  'Knowledge', 'Settings',
]

const BRAIN_LOG = [
  { time: '09:41', type: 'retrieve' as const, text: 'Retrieved 47 product docs + Q3 campaign history from knowledge base' },
  { time: '09:41', type: 'thinking' as const, text: 'Analyzing target audience signals: mid-market SaaS, 50–500 employees, EU/US regions' },
  { time: '09:42', type: 'action' as const, text: 'Generating campaign brief using Campaign Planner module' },
  { time: '09:42', type: 'output' as const, text: 'Draft brief ready — confidence 92% — pending human approval' },
  { time: '09:40', type: 'thinking' as const, text: 'SEO gap analysis: identified 12 low-competition keywords with high intent' },
  { time: '09:38', type: 'action' as const, text: 'Persona Builder: updated ICP "Tech Lead EMEA" with latest CRM signals' },
]

const TASKS = [
  { title: 'Generate Q4 SaaS campaign brief', status: 'running' as const, priority: 'high' as const },
  { title: 'Update SEO meta for 8 product pages', status: 'queued' as const },
  { title: 'Draft email sequence — cold lead revival', status: 'queued' as const },
  { title: 'AEO optimization for FAQ section', status: 'queued' as const, priority: 'high' as const },
  { title: 'Competitor pricing matrix update', status: 'queued' as const },
  { title: 'Instagram ad copy — Q4 launch', status: 'queued' as const },
  { title: 'Brand tone analysis — new website copy', status: 'queued' as const },
]
              ...
              <div key={c.name} className="flex items-center gap-4 p-3.5 rounded-xl bg-slate-50 hover:bg-slate-100 transition-colors cursor-default">
                ...
                {c.health > 0 && (
                  <div className="w-20 hidden lg:block">
                    <ConfidenceMeter value={c.health} color="#7C3AED" />
                  </div>
                )}
              </div>
            ))}
  ...
  return (
    <div className="grid grid-cols-3 gap-6">
      <div className="col-span-2 space-y-5">
        <Card className="p-6">
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-2">
              ...
              <button className="text-xs text-[#7C3AED] font-medium flex items-center gap-1">
        ...
        <Card className="p-6">
          ...
          <div className="relative">
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                { phase: 'Awareness', weeks: 'Wk 1–4', status: 'planned', desc: 'LinkedIn + Meta reach campaigns' },
                { phase: 'Consideration', weeks: 'Wk 5–8', status: 'planned', desc: 'Email nurture + retargeting' },
                { phase: 'Conversion', weeks: 'Wk 9–11', status: 'planned', desc: 'Demo outreach + CTWA' },
                { phase: 'Retention', weeks: 'Wk 12–13', status: 'planned', desc: 'Onboarding + upsell flows' },
              ].map((p, i) => (
        ...
        <Card className="p-5">
          <SectionTitle className="mb-4">Risk Analysis</SectionTitle>
          <div className="space-y-3">
            {[
              { risk: 'Low engagement in EMEA LinkedIn segment', severity: 'Medium', action: 'Adjust messaging tone for EU market' },
              { risk: 'Meta CPC increase in Q4 (seasonal)', severity: 'Low', action: 'Front-load budget to October' },
              { risk: 'Email deliverability for cold outreach', severity: 'Medium', action: 'Warm up domain, limit daily sends' },
            ].map(r => (
              <div key={r.risk} className="flex items-start gap-3 p-3.5 rounded-xl bg-slate-50">
                <AlertCircle size={14} className="text-amber-500 mt-0.5 flex-shrink-0" />
                <div className="flex-1">
                  <div className="text-xs font-medium text-slate-700">{r.risk}</div>
                  <div className="text-xs text-slate-400 mt-0.5">{r.action}</div>
                </div>
                <Badge variant={r.severity === 'Medium' ? 'warning' : 'default'}>{r.severity}</Badge>
              </div>
            ))}
          </div>
        </Card>
          ...
          <div className="space-y-2.5">
            {[
              ...
              <div key={a.item} className="flex items-center justify-between">
                ...
                <Badge variant={a.status === 'approved' ? 'success' : a.status === 'pending' ? 'warning' : 'default'}>
                  {a.status}
                </Badge>
              </div>
            ))}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-6">
        {tools.map(t => (
          <CardHover key={t.label} className="p-5 cursor-pointer group">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center mb-3 transition-transform group-hover:scale-110" style={{ background: t.bg, color: t.color }}>
              {t.icon}
            </div>
    ...
    <div className="grid grid-cols-3 gap-6">
      <div className="col-span-2 space-y-5">
        ...
        <Card className="p-5">
          ...
          <div className="space-y-3">
            {[
              { keyword: 'AI lead automation software', volume: '4,400/mo', difficulty: 32, intent: 'Commercial', opportunity: 'High' },
              { keyword: 'sales agent AI platform', volume: '2,900/mo', difficulty: 28, intent: 'Commercial', opportunity: 'High' },
              { keyword: 'best CRM automation tool 2026', volume: '1,800/mo', difficulty: 41, intent: 'Commercial', opportunity: 'Medium' },
              { keyword: 'AI for marketing automation', volume: '6,600/mo', difficulty: 55, intent: 'Informational', opportunity: 'Medium' },
              { keyword: 'lead scoring automation', volume: '880/mo', difficulty: 19, intent: 'Commercial', opportunity: 'High' },
            ].map(k => (
        ...
        <Card className="p-5">
          <SectionTitle className="mb-4">On-Page Optimization</SectionTitle>
          <div className="space-y-3">
            {[
              { item: 'Meta Title', status: 'Optimized', score: 92 },
              { item: 'Meta Description', status: 'Needs Update', score: 64 },
              { item: 'H1 Structure', status: 'Optimized', score: 88 },
              { item: 'Internal Linking', status: 'In Progress', score: 71 },
              { item: 'Page Speed', status: 'Action Needed', score: 58 },
              { item: 'Schema Markup', status: 'Optimized', score: 95 },
            ].map(item => (
              <div key={item.item} className="flex items-center gap-4">
                <span className="text-xs text-slate-600 w-36 flex-shrink-0">{item.item}</span>
                <div className="flex-1">
                  <ConfidenceMeter value={item.score} />
                </div>
                <Badge variant={item.score >= 80 ? 'success' : item.score >= 65 ? 'warning' : 'error'}>
                  {item.status}
                </Badge>
              </div>
            ))}
  ...
  return (
    <div className="grid grid-cols-3 gap-6">
      <div className="col-span-2 space-y-5">
        ...
        <Card className="p-5">
          <SectionTitle className="mb-4">FAQ Optimization</SectionTitle>
          <div className="space-y-3">
            {[
              { q: 'What is AI lead automation?', status: 'Optimized', citability: 88 },
              { q: 'How does Orbq qualify leads automatically?', status: 'Optimized', citability: 82 },
              { q: 'What AI agents does Orbq include?', status: 'In Progress', citability: 64 },
              { q: 'How is Orbq different from HubSpot AI?', status: 'Needs Work', citability: 41 },
            ].map(f => (
              <div key={f.q} className="p-3.5 rounded-xl bg-slate-50">
                <div className="flex items-start justify-between mb-2">
                  <span className="text-xs text-slate-700 flex-1 pr-3">{f.q}</span>
                  <Badge variant={f.status === 'Optimized' ? 'success' : f.status === 'In Progress' ? 'warning' : 'error'}>
                    {f.status}
                  </Badge>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-slate-400">Citability:</span>
                  <div className="flex-1">
                    <ConfidenceMeter value={f.citability} />
                  </div>
                  <Mono color={f.citability >= 80 ? '#10B981' : f.citability >= 60 ? '#F59E0B' : '#EF4444'}>{f.citability}%</Mono>
                </div>
              </div>
            ))}
          </div>
  ...
  const personas = [
    {
      name: 'Tech Lead · EMEA',
      role: 'Head of RevOps',
      company: 'SaaS 50–200 employees',
      goals: 'Automate manual CRM tasks, reduce sales cycle',
      pains: 'Too much manual data entry, poor lead quality from marketing',
    ...
    {
      name: 'CMO · Growth Stage',
      role: 'Chief Marketing Officer',
      company: 'B2B SaaS 200–500 employees',
      goals: 'Scale content output, improve MQL quality',
      pains: 'Content team bottleneck, attribution unclear',
      channels: 'Email, Events, Paid Search',
      conversion: 52,
      color: '#0284C7',
      bg: '#F0F9FF',
    },
    {
      name: 'Sales Director · US',
      role: 'VP of Sales',
      company: 'Enterprise 500+ employees',
      goals: 'Increase win rate, shorten deal cycles',
      pains: 'Reps waste time on unqualified leads',
      channels: 'Outbound, Referrals, LinkedIn',
      conversion: 74,
      color: '#059669',
      bg: '#F0FDF4',
    },
        ...
        <div className="grid grid-cols-5 gap-4">
          {['Awareness', 'Interest', 'Consideration', 'Intent', 'Purchase'].map((stage, i) => (
            <div key={stage} className="text-center">
              <div className="w-8 h-8 rounded-full mx-auto mb-2 flex items-center justify-center font-bold text-white text-xs"
                style={{ background: `hsl(${220 + i * 20}, 70%, ${50 - i * 3}%)` }}>
                {i + 1}
              </div>
              <div className="text-xs font-semibold text-[#0F1929] mb-1">{stage}</div>
              <div className="text-xs text-slate-400 leading-relaxed">
                {['Content discovery · SEO', 'Webinar · Blog', 'Demo request · Trial', 'Proposal · Pricing', 'Signed contract'][i]}
              </div>
            </div>
          ))}
        </div>
    ...
    <div className="space-y-6">
      <div className="p-4 rounded-2xl border border-amber-200 bg-amber-50 flex items-start gap-3">
        ...
        <p className="text-xs text-amber-700 leading-relaxed">
          <strong>Disclaimer:</strong> Insights are AI-generated from publicly available data and should be verified before strategic decisions. Last updated: Aug 3, 2026.
        </p>
      </div>
        ...
        <table className="w-full min-w-[600px]">
          <thead>
            <tr>
              ...
              {competitors.map(c => (
                <th key={c} className="text-center text-xs text-slate-500 font-medium pb-3 px-3">{c}</th>
              ))}
            </tr>
          </thead>
      ...
      <div className="grid grid-cols-2 gap-6">
        <Card className="p-5">
          <SectionTitle className="mb-4">SWOT Analysis (vs. Market)</SectionTitle>
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: 'Strengths', items: ['Unified 3-agent architecture', 'RAG-grounded responses', 'CTWA + WhatsApp native'], color: '#10B981', bg: '#F0FDF4' },
              { label: 'Weaknesses', items: ['Smaller brand awareness', 'Fewer native integrations', 'No self-serve tier'], color: '#EF4444', bg: '#FFF1F2' },
              { label: 'Opportunities', items: ['AEO is an untapped space', 'EMEA mid-market under-served', 'AI agent demand surging'], color: '#3B6EF0', bg: '#EEF2FF' },
              { label: 'Threats', items: ['HubSpot expanding AI fast', 'Price wars in CRM market', 'OpenAI enterprise push'], color: '#F59E0B', bg: '#FFFBEB' },
            ].map(q => (
              <div key={q.label} className="p-3.5 rounded-xl" style={{ background: q.bg }}>
        ...
        <Card className="p-5">
          <SectionTitle className="mb-4">Positioning Map</SectionTitle>
          <div className="relative h-48 bg-slate-50 rounded-xl border border-[#E4E8F0] overflow-hidden">
            {/* Axes */}
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-px h-full bg-slate-200 absolute left-1/2" />
              <div className="h-px w-full bg-slate-200 absolute top-1/2" />
            </div>
            <span className="absolute top-2 left-1/2 -translate-x-1/2 text-xs text-slate-400">Enterprise</span>
            <span className="absolute bottom-2 left-1/2 -translate-x-1/2 text-xs text-slate-400">SMB</span>
            <span className="absolute left-2 top-1/2 -translate-y-1/2 text-xs text-slate-400" style={{ writingMode: 'vertical-rl', transform: 'rotate(180deg) translateY(50%)' }}>Basic AI</span>
            <span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-slate-400" style={{ writingMode: 'vertical-rl' }}>Advanced AI</span>

            {/* Competitor dots */}
            {[
              { name: 'Orbq', x: 72, y: 28, color: '#7C3AED', size: 14 },
              { name: 'Salesforce', x: 60, y: 20, color: '#0284C7', size: 12 },
              { name: 'HubSpot', x: 52, y: 52, color: '#F59E0B', size: 12 },
              { name: 'Marketo', x: 40, y: 35, color: '#EF4444', size: 10 },
              { name: 'Lemlist', x: 65, y: 70, color: '#10B981', size: 9 },
            ].map(d => (
              <div key={d.name}
                className="absolute -translate-x-1/2 -translate-y-1/2 rounded-full flex items-center justify-center text-white font-bold"
                style={{ left: `${d.x}%`, top: `${d.y}%`, width: d.size * 2, height: d.size * 2, background: d.color, fontSize: 9 }}
                title={d.name}
              >
                {d.name.charAt(0)}
              </div>
    ...
    <div className="grid grid-cols-3 gap-6">
      <div className="col-span-2 space-y-5">
        <Card className="p-6">
          ...
          <div className="space-y-4">
            {[
              { label: 'Ad Objective', value: 'Lead Generation via WhatsApp', type: 'select' },
              { label: 'Headline', value: 'Automate Your Sales Pipeline — AI Does the Heavy Lifting', type: 'text' },
              { label: 'Primary Text', value: 'Stop chasing cold leads. Orbq\'s AI agents qualify, score, and engage leads automatically — 24/7. Start your free trial.', type: 'textarea' },
              { label: 'Description', value: 'Trusted by 400+ B2B teams · RAG-powered · No credit card required', type: 'text' },
              { label: 'WhatsApp Greeting Message', value: 'Hi! I saw your ad and I\'m interested in automating my sales team. Can you tell me more?', type: 'textarea' },
              { label: 'CTA Button Text', value: 'Chat on WhatsApp', type: 'select' },
            ].map(f => (
              <div key={f.label} className="grid grid-cols-3 gap-4">
                <Label>{f.label}</Label>
                <div className="col-span-2">
                  {f.type === 'textarea' ? (
                    <div className="p-3 bg-slate-50 rounded-xl text-xs text-slate-700 leading-relaxed">{f.value}</div>
                  ) : (
                    <div className="px-3 py-2 bg-slate-50 rounded-xl text-xs text-slate-700">{f.value}</div>
                  )}
                </div>
              </div>
            ))}
        ...
        <Card className="p-5">
          <SectionTitle className="mb-4">Qualification Questions (WhatsApp Flow)</SectionTitle>
          <div className="space-y-2.5">
            {[
              'What is your company size?',
              'Are you currently using a CRM? If yes, which one?',
              'What is your biggest sales challenge right now?',
              'How many leads do you process per month?',
            ].map((q, i) => (
              <div key={i} className="flex items-start gap-3 p-3.5 rounded-xl bg-green-50">
                <span className="w-5 h-5 rounded-full bg-green-500 flex items-center justify-center text-white font-bold text-xs flex-shrink-0">
                  {i + 1}
                </span>
      ...
      <div className="space-y-5">
        ...
        <Card className="p-5">
          ...
          <div className="rounded-xl overflow-hidden border border-[#E4E8F0]">
            <div className="bg-slate-100 h-28 flex items-center justify-center text-slate-300 text-sm">
              [Ad Creative]
            </div>
            <div className="p-3 bg-white">
              <div className="text-xs text-slate-400 mb-0.5">orbq.ai · Sponsored</div>
              <div className="text-sm font-semibold text-[#0F1929] leading-tight">
                Automate Your Sales Pipeline — AI Does the Heavy Lifting
              </div>
              <div className="text-xs text-slate-500 mt-1">
                Trusted by 400+ B2B teams · RAG-powered
              </div>
              <button className="mt-2 w-full py-2 rounded-lg bg-green-500 text-white text-xs font-semibold">
                Chat on WhatsApp
              </button>
            </div>
          </div>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setView('month')} className={`p-2 rounded-lg transition-all ${view === 'month' ? 'bg-purple-50 text-purple-600' : 'text-slate-400 hover:bg-slate-50'}`}>
            <LayoutGrid size={15} />
          </button>
          <button onClick={() => setView('list')} className={`p-2 rounded-lg transition-all ${view === 'list' ? 'bg-purple-50 text-purple-600' : 'text-slate-400 hover:bg-slate-50'}`}>
            <List size={15} />
          </button>
        </div>
      </div>
        ...
        {view === 'month' ? (
          <>
            ...
            <div className="grid grid-cols-7 gap-1">
              {days.map(day => (
                <div key={day}
                  className={`min-h-[64px] p-1.5 rounded-xl border text-xs ${day === 3 ? 'border-purple-200 bg-purple-50' : 'border-transparent hover:border-[#E4E8F0]'} transition-all cursor-default`}>
                  <div className={`text-xs font-medium mb-1 ${day === 3 ? 'text-purple-700' : 'text-slate-500'}`}>{day}</div>
                  {events[day]?.map(e => (
                    <div key={e.label} className="text-xs px-1.5 py-0.5 rounded mb-0.5 truncate text-white" style={{ background: e.color, fontSize: 9 }}>
                      {e.label}
                    </div>
                  ))}
                </div>
...
export default function MarketingWorkspace({ onNavigate: _onNavigate }: { onNavigate: (v: AppView) => void }) {
  ...
  return (
    <div className="min-h-screen">
      <WorkspaceHeader
        ...
        icon="📣"
        badge="Running · 7 tasks queued"
        confidence={92}
        color="#7C3AED"
        bgColor="#F5F3FF"
        description="Campaign automation, content generation, SEO/AEO, and market intelligence"
      />
      <div className="px-8">
