...
router.post('/', authenticate, requirePermission('campaigns:write'), withTenantScope, async (req, res) => {
  try {
    ...
    const result = await query(
      ...
      [req.organizationId, name, category, channel, content, is_public]
    );
  ...
  try {
    ...
    const result = await query(
      `UPDATE mh_templates 
       ...
       RETURNING *`,
      [name, category, channel, content, is_public, req.params.id, req.organizationId]
    );
...
router.post('/:id/use', authenticate, requirePermission('campaigns:write'), withTenantScope, async (req, res) => {
  try {
    const result = await query(
      `UPDATE mh_templates 
