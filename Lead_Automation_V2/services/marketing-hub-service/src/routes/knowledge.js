...
router.post('/', authenticate, requirePermission('campaigns:write'), withTenantScope, async (req, res) => {
  try {
    ...
    const result = await query(
      ...
      [
        req.organizationId, title, content, category, tags || [],
  ...
  try {
    ...
    const result = await query(
      ...
      [
        ...
        JSON.stringify(metadata || {}), req.params.id, req.organizationId
      ]
    );
...
router.post('/search', authenticate, requirePermission('campaigns:read'), withTenantScope, async (req, res) => {
  try {
    ...
    const result = await query(
      `SELECT *, ts_rank(search_vector, plainto_tsquery('english', $2)) as rank
       ...
       LIMIT $3`,
      [req.organizationId, searchQuery, parseInt(limit)]
    );
...
router.get('/categories/list', authenticate, requirePermission('campaigns:read'), withTenantScope, async (req, res) => {
  try {
    const result = await query(
...
router.get('/popular/list', authenticate, requirePermission('campaigns:read'), withTenantScope, async (req, res) => {
  try {
    ...
    const result = await query(
      `SELECT id, title, category, view_count, helpful_count, created_at
       ...
       LIMIT $2`,
      [req.organizationId, parseInt(limit)]
    );
...
router.post('/:id/helpful', authenticate, requirePermission('campaigns:read'), withTenantScope, async (req, res) => {
  try {
    const result = await query(
      `UPDATE mh_knowledge_articles 
