...
async function getValidAccessToken(organizationId) {
  ...
  try {
    const creds = await configStore.resolveCredentials(organizationId);
    if (!creds) {
      const err = new Error('Google OAuth is not configured for this organization. Save a Client ID and Client Secret first.');
      err.status = 400;
      throw err;
    }
    const refreshed = await refreshAccessToken(decrypt(row.refresh_token), creds);
